...
export function useLessonProgress() {
  ...
  const saveLessonCompletion = useCallback(async (
    lessonId: string,
    submoduleId: string,
    moduleId: string,
    totalPages: number
    ...
    try {
      ...
      const { data: { user }, error: authError } = await progressClient.auth.getUser();
      ...
      await cachedProgressService.updateProgressOnLessonCompletion(moduleId, submoduleId);
