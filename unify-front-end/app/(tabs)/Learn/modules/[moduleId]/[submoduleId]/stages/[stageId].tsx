...
export default function StageScreen() {
  ...
  const { moduleId, submoduleId, stageId } = useLocalSearchParams<{
    moduleId: string;
    submoduleId: string;
    stageId: string;
  }>();
  ...
  const {
    ...
    error: stageError,
  } = useStageInfo(stageId || '');
  const { data: lessons, isLoading: loadingLessons } = useStageLessons(
    stageId || ''
  );
  ...
  const completedLessons =
    lessons?.filter((l: any) => l.is_completed)?.length || 0;
    ...
    <SafeAreaView style={styles.safe}>
      ...
      >
        ...
        <Text style={styles.title}>
          Welcome to Stage {stage.order_num} of Your{' '}
          {moduleData?.title || 'Finance'} Journey!
        </Text>
