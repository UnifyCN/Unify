const projectId = process.env.EXPO_PUBLIC_SANITY_PROJECT_ID || process.env.SANITY_PROJECT_ID;
const dataset = process.env.EXPO_PUBLIC_SANITY_DATASET || process.env.SANITY_DATASET || 'production';
...
export const imageBuilder = {
    image: (source: any) => {
        if (source && source.asset && source.asset._ref) {
            const imageId = source.asset._ref.replace('image-', '').replace('-jpg', '.jpg').replace('-png', '.png').replace('-webp', '.webp');
