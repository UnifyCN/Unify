...
export function useAnalytics() {
  ...
  return useMemo(() => ({
    ...
    trackPostCommentOpened: (postId: string) => {
      posthog?.capture(AnalyticsEvents.POST_COMMENT_OPENED, { post_id: postId });
    },
    ...
    trackLessonCompleted: (moduleId: string, submoduleId: string, lessonId: string) => {
    ...
    trackSubmoduleViewed: (moduleId: string, submoduleId: string, submoduleTitle?: string) => {
    ...
    trackLessonStarted: (moduleId: string, submoduleId: string, lessonId: string, lessonTitle?: string) => {
    ...
    trackActivityCompleted: (moduleId: string, submoduleId: string, lessonId: string, activityId: string) => {
    ...
    trackSignUpFailed: (errorType: string) => {
      posthog?.capture(AnalyticsEvents.SIGN_UP_FAILED, { error_type: errorType });
    },
    trackSignInCompleted: () => {
      posthog?.capture(AnalyticsEvents.SIGN_IN_COMPLETED);
    },
    trackSignInFailed: (errorType: string) => {
      posthog?.capture(AnalyticsEvents.SIGN_IN_FAILED, { error_type: errorType });
    },
    trackGoogleSignInUsed: (authType: 'sign_up' | 'sign_in') => {
      posthog?.capture(AnalyticsEvents.GOOGLE_SIGN_IN_USED, { auth_type: authType });
    },
    ...
    capture: (eventName: AnalyticsEventName, properties?: Record<string, any>) => {
