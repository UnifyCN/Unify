// @ts-nocheck Deno edge runtime + JSR imports are not resolved by workspace TypeScript.
  ...
  async function buildChecklistBlock(
    ...
    const items = sorted.map(x => {
      ...
      return {
        ...
        why_tag: personalized ? whyTagChecklist(profile as ProfileRow) : null,
    ...
    if (personalized) {
      ...
      await logRecommendationEvents(supabase, userId, logEntries);
    }
  ...
  async function buildLearnBlock(
    ...
    const modules = sorted.map((x, i) => {
      ...
      return {
        ...
        score,
        progress,
        why_tag: personalized ? whyTagLearn(profile as ProfileRow) : null,
      };
    });
