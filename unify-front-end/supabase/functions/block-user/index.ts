...
Deno.serve(async (req) => {
  if (!RESEND_API_KEY || !RESEND_FROM || !RESEND_TO) {
    ...
    return new Response(JSON.stringify({ error: "Missing Resend env vars" }), { status: 500 });
  }
  ...
  try {
    ...
    if (!blockedUserId) {
      return new Response(JSON.stringify({ success: false, error: "Missing blockedUserId" }), { status: 200 });
    }

    const token = req.headers.get("Authorization")?.replace("Bearer ", "");
    if (!token) return new Response(JSON.stringify({ success: false, error: "Unauthorized" }), { status: 200 });

    const { data: authData, error: userError } = await supabase.auth.getUser(token);
    const user = authData?.user;
    if (userError || !user) return new Response(JSON.stringify({ success: false, error: "Invalid user" }), { status: 200 });

    // Prevent self-blocking
    if (user.id === blockedUserId) {
      return new Response(JSON.stringify({ success: false, error: "Cannot block yourself" }), { status: 200 });
    }
    ...
    if (existingBlock) {
      return new Response(JSON.stringify({ success: false, error: "Already blocked" }), { status: 200 });
    }
    ...
    if (insertError) {
      ...
      return new Response(JSON.stringify({ success: false, error: "Failed to block user" }), { status: 200 });
    }
