...
class CachedProgressService {
  ...
  async getProgressData(forceRefresh = false): Promise<CachedProgressData> {
    ...
    if (!forceRefresh && (now - this.lastFetch) < this.CACHE_DURATION && Object.keys(this.cache).length > 0) {
    ...
    try {
      ...
      modulesData.forEach((module: any) => {
        ...
        module.submodules.forEach((submodule: any) => {
          ...
          const completedLessons = lessonProgresses?.filter((lesson: any) => 
            lesson.sanity_submodule_id === submodule._id && lesson.is_completed
          ).length || 0;
  ...
  async updateProgressOnLessonCompletion(moduleId: string, submoduleId: string) {
