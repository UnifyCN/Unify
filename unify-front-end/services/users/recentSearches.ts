...
export async function saveRecentSearch(userId: string, inputQuery: string) {
  ...
  if (searchErr) return { error: searchErr };

  //remove dupe
  const found = searchExists?.find(
    row => row.search_query.trim().toLowerCase() === query
  );
  if (found) {
    const { error: foundErr } = await supabase
      ...
      .delete()
      .eq('id', found.id);
    if (foundErr) return { error: foundErr };
  }
  ...
  const { data: after, error: refetchErr } = await supabase
    ...
    .eq('user_id', userId)
    .order('created_at', { ascending: false });
  if (refetchErr) return { error: refetchErr };
