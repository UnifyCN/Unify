...
export async function uploadProfilePicture(
  ...
  try {
    ...
    // 1) sign PUT
    const signResp = await fetch(`${SUPABASE_URL}/functions/v1/profile-picture-upload`, {
      method: "POST",
      headers: {
    ...
    if (!signResp.ok) {
      throw new Error(signJson?.error ?? `Failed to sign upload (${signResp.status})`);
    }
...
export async function getProfilePictureUrl(key: string): Promise<string> {
  ...
  if (!resp.ok) throw new Error(json?.error ?? `Failed to sign GET (${resp.status})`);
...
export async function deleteProfilePicture(key: string): Promise<UploadResult> {
  try {
    ...
    const resp = await fetch(`${SUPABASE_URL}/functions/v1/profile-picture-remove`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
    ...
    const json = await resp.json().catch(() => ({}));
    if (!resp.ok) throw new Error(json?.error ?? `Failed to sign delete (${resp.status})`);
